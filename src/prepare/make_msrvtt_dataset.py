...
def main(args):
  ...
  if df_idx == 0:
    ...
    print("Number of captions remaining: {}".format(len(data['sentences']) - len(df)))
  
  for i in range(len(data['sentences'])):
    if i % 1000 == 0:
      print ("Converting json to csv: {}%\r".format(round(i/float(len(data['sentences']))*100,2)),end='')
    ...
    df.loc[df_idx,'caption'] = unicodeToAscii(data['sentences'][i]['caption'])
    df_idx += 1
  ...
  for i, row in df.iterrows():
    if i % 1000 == 0:
      print ("Assigning to sets: {}%\r".format(round(i/float(len(df))*100,2)),end='')
      df.to_csv(args.interim_data_path + 'partially_distributed_captions.csv',index=False)
  ...
  print ("Saved final distributed set to " + args.final_data_path + 'msrvtt_captions.csv')
  print ('\nDone')

if __name__ == '__main__':
  parser = argparse.ArgumentParser()
  parser.add_argument('--train_pct',type=float,help='Percentage of dataset to use for training',
                      default=0.8)
  parser.add_argument('--dev_pct',type=float,help='Percentage of dataset to use for development',
                      default=0.15)
  parser.add_argument('--raw_data_path',type=str, help='Path to raw datafile',
                      default='data/raw/videodatainfo_2017_ustc.json')
  parser.add_argument('--interim_data_path',type=str, help='Path to interim datafile',
                      default='data/interim/')
  parser.add_argument('--final_data_path',type=str, help='Path to final datafile',
                      default='data/processed/') 
  parser.add_argument('--continue_converting', type-bool, help='Continue converting json to csv',
                      default=True)
