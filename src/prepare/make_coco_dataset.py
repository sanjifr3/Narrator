...
def main(args):
  ...
  for st in set_2014 + set_2017:
    ...
    filenames = [(int(img['id']), st + '/' + img['file_name']) for img in imgs]
    ...
    df.to_csv(args.interim_result_path + 'coco_' + st + '_captions.csv',index=False)

  print('\nDone Converting')
  print('Number of images: {}'.format(cocoDF['id'].nunique()))

  cocoDF.to_csv(args.coco_path + 'annotations/coco_captions.csv', index=False)
  #cocoDF.to_csv(args.result_path + 'coco_captions.csv',index=False)

  print("Saved merged set to " + args.coco_path + 'annotations/coco_captions.csv')

  # Make 2014 and 2017 dataframes
  val2014DF = pd.read_csv(args.interim_result_path + 'coco_val2014_captions.csv')
  val2017DF = pd.read_csv(args.interim_result_path + 'coco_val2017_captions.csv')
  train2014DF = pd.read_csv(args.interim_result_path + 'coco_train2014_captions.csv')
  train2017DF = pd.read_csv(args.interim_result_path + 'coco_train2017_captions.csv')
...
if __name__ == '__main__':
  ...
  parser.add_argument('--interim_result_path',type=str, help='Path to interim datafile',
                      default='data/interim/')
  parser.add_argument('--results_path',type=str, help='Path to year datafile',
