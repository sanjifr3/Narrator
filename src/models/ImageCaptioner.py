...
class ImageCaptioner(nn.Module):
  def __init__(self, im_embedding_size, embed_size, hidden_size, vocab_size, 
    ...
    # LSTM
    self.rnn = rnn_type(embed_size, hidden_size, num_layers, batch_first=True, dropout=(0 if num_layers == 1 else rnn_dropout_prob))
  ...
  def forward(self, im_embeddings, caption_embeddings=None, mode='train'):
    ...
    if mode=='test':
      return predict(im_embeddings)
    
    # Prepare image for passing into first RNN
  ...
  def predict(self, im_embeddings, return_probs=False, beam_size=None, desired_num_captions=1):
    ...
    if not beam_size:
        ...
        for batch_id in range(batch_size):
            if return_probs:
                captions[batch_id], probs[batch_id] = self.beamSearch(im_embeddings[batch_id].unsqueeze(0),
                                                                      return_probs=return_probs,
                                                                      beam_size=beam_size,
                                                                      topK=desired_num_captions)
            else:
                captions[batch_id] = self.beamSearch(im_embeddings[batch_id].unsqueeze(0),
                                                      return_probs=return_probs,
                                                      beam_size=beam_size,
                                                      topK=desired_num_captions)
