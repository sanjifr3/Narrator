...
class VideoCaptioner(nn.Module):
    ...
    def __init__(self, vid_embedding_size, embed_size, hidden_size,
        ...
        # Store inputs as class variables
        ...
        # Layers for preprocessing video
        ...
        # Layers for preprocessing caption
        self.embed = nn.Embedding(vocab_size, embed_size)

        # Encoding LSTM
        self.inp_rnn = rnn_type(embed_size, hidden_size, num_layers,
                                batch_first=True,
                                dropout=(0 if num_layers == 1
                                         else rnn_dropout_prob))

        # Decoding LSTM
        self.out_rnn = rnn_type(embed_size, hidden_size, num_layers,
                                batch_first=True,
                                dropout=(0 if num_layers == 1
                                         else rnn_dropout_prob))
    ...
    def forward(self, vid_embeddings, caption_embeddings=None, mode='train'):
        ...
        # Pass through encoding RNN and decoding RNN
        inp_outputs, hidden = self.inp_rnn(vid_embeddings)
        outputs, _ = self.out_rnn(caption_embeddings, hidden)

        # Pass through linear output
        #outputs = self.out(outputs)
        outputs = torch.cat(self.out(inp_outputs[:, -1]), 
                            self.out(outputs))
        return outputs
    ...
    def predict(self, vid_embeddings, return_probs=False,
        ...
        if not beam_size:
            inp_output, hidden = self.inp_rnn(vid_embeddings)

            captions = torch.zeros(batch_size, self.max_len)
            captions[:, 0] = self.start_id
            if return_probs:
                probs = torch.zeros(batch_size, self.max_len, 
                                    self.output_size)
                probs[:, 0] = self.out(
                                    inp_output[:, -1].squeeze(1)).cpu()
            ...
            for i in range(self.max_len):
                ...
                output, hidden = self.out_rnn(word_embedding, hidden)
            ...
            _, hidden = self.inp_rnn(vid_embeddings)
            ...
            for batch_id in range(batch_size):
                if return_probs:
                    captions[batch_id], probs[batch_id] = self.beamSearch(word_embedding[batch_id].unsqueeze(0).unsqueeze(0),
                                                                          (hidden[0][:, batch_id].unsqueeze(
                                                                              0), hidden[1][:, batch_id].unsqueeze(0)),
                                                                          return_probs=return_probs,
                                                                          beam_size=beam_size,
                                                                          topK=desired_num_captions)
                else:
                    captions[batch_id] = self.beamSearch(word_embedding[batch_id].unsqueeze(0).unsqueeze(0),
                                                         (hidden[0][:, batch_id].unsqueeze(
                                                             0), hidden[1][:, batch_id].unsqueeze(0)),
                                                         return_probs=return_probs,
                                                         beam_size=beam_size,
    ...
    def beamSearch(self, output, hidden=None,
        ...
        for i in range(self.max_len):
            ...
            for idx_seq in idx_sequences:
                ...
                for j in range(beam_size):
                    ...
                    if return_probs:
                        candidates.append(
                            [next_idx_seq, log_prob, output, hidden, idx_seq[4]])
                    else:
                        candidates.append(
        ...
        if return_probs:
            return torch.Tensor([idx_seq[0] for idx_seq in idx_sequences[:topK]]), torch.Tensor(
                [idx_seq[4] for idx_seq in idx_sequences[:topK]])
        else:
