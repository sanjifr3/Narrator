...
class YOLOObjectDetector(object):
  ...
  def __init__(self, darknet_path=os.environ['HOME'] + '/programs/darknet/',
    ...
    meta_file = '%s/data/%s.data' % (darknet_path, dataset)
    ...
    self.colors = pickle.load(open(dir_name + "/pytorch-yolo-v3/pallete", "rb"))
    ...
    self.model(self.getTestInput(), torch.cuda.is_available())
    self.model.eval()

    self.transformer = createYoloTransformer(self.dim)
    return

  def detect(self, im, thresh=0.3, nms_thresh=0.45, max_objects=4, draw=False):
    im, orig_im, dim = self.prepImage(im)
    ...
    output = write_results(output, thresh, len(self.classes), nms=True, nms_conf=nms_thresh)
    ...
    if type(output) != int:
      ...
      output[:,[1,3]] -= (self.dim - scaling_factor*im_dim[:,0].view(-1,1))/2
      output[:,[2,4]] -= (self.dim - scaling_factor*im_dim[:,1].view(-1,1))/2
      ...
      if output.size()[0] > 0:
        if max_objects != None and output.size()[0] > max_objects:
        ...
        classes = self.np_classes[output[:,-1].int().cpu().numpy().tolist()].tolist()
        output = output[:,1:7]
    ...
    if draw:
      return objects, orig_im

    return classes, output

  def getObjInfo(self, x):
    c1 = tuple(x[1:3].int().cpu().numpy())
    c2 = tuple(x[3:5].int().cpu().numpy())
    cls = int(x[-1])
    label = "{0}".format(self.classes[cls])

    obj_conf = x[5].cpu().numpy()
    cls_conf = x[6].cpu().numpy()

    x1 = c1[0]

    return label, cls_conf, obj_conf, c1, c2    
