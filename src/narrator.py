...
from features.create_transformer import createTransformer
...
class Narrator(object):
    ...
    def __init__(self):
        with open(self.coco_vocab_path, 'rb') as f:
            self.coco_vocab = pickle.load(f)
        with open(self.msrvtt_vocab_path, 'rb') as f:
            self.msrvtt_vocab = pickle.load(f)

        self.transformer = createTransformer()
        self.encoder = EncoderCNN(self.base_model)

        self.image_captioner = ImageCaptioner(
            self.im_embedding_size,
            self.embed_size,
            self.hidden_size,
            len(self.coco_vocab),
        ...
        self.image_captioner.load_state_dict(self.ic_model_path)

        self.video_captioner = VideoCaptioner(
            self.vid_embedding_size,
            self.embed_size,
            self.hidden_size,
        ...
        self.video_captioner.load_state_dict(self.vc_model_path)
    ...
    def genCaption(self, f, beam_size=5, as_string=False, save_path=None):
        ext = f.split('.')[-1]
        if ext in self.img_extensions:
            return self.genImCaption(f, beam_size, as_string, save_path)
    ...
    def genImCaption(self, f, beam_size=5, as_string=False, save_path=None):
        if os.path.exists(f):
        ...
        caption = self.coco_vocab.decode(caption, clean=True)
    ...
    def saveAudio(self, text, path):
        tts.generateAudio(text, path)
        return
